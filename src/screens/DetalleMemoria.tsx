...
const DetalleMemoria = ({ route, navigation }) => {
  const { memoriaId } = route.params;
  const memoria = memorias.find(m => m.id === memoriaId);


  const playSong = () => {
    navigation.navigate('Reproductor', { memoriaId: memoria.id });
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>{memoria.tituloMemoria}</Text>
      <Text style={styles.description}>{memoria.descripcion}</Text>
      <Text style={styles.date}>{memoria.fecha}</Text>
      <ItemSong
        song={memoria.cancion}
        artist={memoria.artista}
        onPlay={playSong}
      />
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  description: {
    marginBottom: 15,
    fontSize: 16,
    height: 150, // Aproximadamente para 500 caracteres
  },
  date: {
    marginBottom: 15,
    fontSize: 14,
